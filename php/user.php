...
class User
{
    ...
    **/
    ...
    function rebuild_database($dbfile)
    {
        ...
        $create .="CREATE TABLE seen (uName TEXT, movieId TEXT, seen BOOLEAN, FOREIGN KEY(uName) REFERENCES users(username)); PRAGMA foreign_keys = ON;";
...
EOT;
    ...
    function register_user($username, $email, $sha1, & $registration_callback = false)
    {
        ...
        //Til að ná í imdbID úr movies.json og lesa inn í seen töfluna.
        $json_data = file_get_contents("../data/movies.json");
        $json_decode = json_decode($json_data, true);
        $i = 0;
        //Setur inn movieID og false í seen töflu
        foreach ($json_decode as $key) {
            $id = $json_decode[$i]["id"]."cbx";
            $insertSeen = "INSERT INTO seen (uName, movieId, seen) "; 
            $insertSeen .= "VALUES ('$username', '$id', 'false');"; 
            $this->database->exec($insertSeen); 
    ...
    function unregister_user($username)
    {
        ...
        $deleteSeen = "DELETE FROM seen WHERE uName='$username'";
        $this->database->exec($delete);
        $this->database->exec($deleteSeen);
